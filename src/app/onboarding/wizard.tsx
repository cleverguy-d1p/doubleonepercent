...
export default function Wizard(){
 ...
 :showPrimer?<main className="auth primer"><h1>BEFORE WE GET STARTED</h1><p>Welcome aboard. Before building your nutrition and training program, complete the intake below as thoroughly as possible.</p><p>Some of it may feel detailed, but the more upfront information, the more specifically we build everything around your body, lifestyle, schedule, food preferences, training experience, and goal of losing body fat, adding lean muscle, and getting you properly conditioned.</p><p>A few notes:</p><ul><li>Don&apos;t worry if you don&apos;t know the answer to everything.</li><li>Take your time filling this out, even small details are much more useful than short answers.</li><li>You can always save your progress and pick up where you left off.</li><li>It should take you about 15-20 mins to complete.</li></ul><p>All of these inputs will be used to start building your initial nutrition, strength, cardio, and training-specific plan. We&apos;ll schedule your launch call approximately 5-7 days later to review everything together.</p><p>We will use your regular check-ins and progress data to make adjustments as we go.</p><p>Looking forward to getting started.</p><button className="button" onClick={()=>setShowPrimer(false)}>Start my intake ↗</button></main>
 ...
 <div className="actions"><button type="button" className="secondary" disabled={busy||uploadCount>0||step===0} onClick={()=>move(step-1)}>← Back</button><button className="button" disabled={busy||uploadCount>0}>{step===lastSectionStep?"Review intake":"Save & continue"} ↗</button></div></form>
 :<div className="review"><p className="intro">Check your answers below. Use Edit to return to a section. Blank fields will be recorded as not provided.</p>{sections.map((s,i)=><section className="review-section" key={s.id}><h2>{s.title}</h2><dl>{s.fields.filter(f=>f.type!=="note"&&visible(f,answers)).map(f=><div key={f.id}><dt>{f.label}</dt><dd>{f.type==="photo"?(answers[f.id]?"Photo uploaded":"Not provided"):answers[f.id]||"Not provided"}</dd></div>)}</dl><button className="secondary" onClick={()=>move(i)}>Edit section</button></section>)}
 <label className="check"><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/><span>I have reviewed my answers and agree to my information being used for coaching as described in the <Link href="/privacy"><u>privacy notice</u></Link>.</span></label><div className="actions"><button className="secondary" disabled={busy||uploadCount>0} onClick={()=>move(lastSectionStep)}>← Back</button><button className="button" disabled={!consent||busy||uploadCount>0||!answers.q719?.trim()} onClick={submit}>Submit my intake ↗</button></div>{!answers.q719?.trim()?<p className="note">Please add your full name in Basic Information before submitting.</p>:null}</div>}
 </section></main>}</>;
}
function Question({field:f,value,onChange,userId,onError,onUploading}:{field:Field;value:string;onChange:(v:string)=>void;userId:string;onError:(v:string)=>void;onUploading:(v:boolean)=>void}){
 ...
 if(f.type==="radio"||f.type==="radio-details"){
  const options=f.options||["No","Yes"];
  const choice=options.find(option=>value===option||value.startsWith(`${option}\n`))||"";
  const details=choice&&value.startsWith(`${choice}\n`)?value.slice(choice.length+1):"";
  const showDetails=f.type==="radio-details"&&choice==="Yes";
  function choose(next:string){
   onChange(f.type==="radio-details"&&next==="Yes"?`${next}\n`:next);
  }
  return <fieldset className="field-group"><legend>{f.label}</legend><div className="choice-list">{options.map(option=><label key={option}><input type="radio" name={f.id} checked={choice===option} onChange={()=>choose(option)}/><span>{option}</span></label>)}</div>{showDetails?<textarea aria-label={f.label} required maxLength={5000} rows={3} value={details} onChange={e=>onChange(`${choice}\n${e.target.value}`)}/>:null}</fieldset>;
 }
 const options=f.type==="scale"?Array.from({length:10},(_,i)=>String(i+1)):f.options;
 return <label className="field"><span>{f.label}</span>{options?<select value={value} onChange={e=>onChange(e.target.value)}><option value="">Select an answer</option>{options.map(o=><option key={o}>{o}</option>)}</select>:f.type==="textarea"?<textarea maxLength={5000} rows={3} value={value} onChange={e=>onChange(e.target.value)}/>:<input type={f.type==="date"?"date":"text"} max={f.type==="date"?new Date().toISOString().slice(0,10):undefined} required={f.id==="q719"} maxLength={500} autoComplete="off" value={value} onChange={e=>onChange(e.target.value)}/>}</label>;
}
