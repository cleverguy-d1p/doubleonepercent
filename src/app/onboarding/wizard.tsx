...
export default function Wizard(){
 ...
 :<div className="review"><p className="intro">Check your answers below. Blank fields will be recorded as not provided.</p><fieldset disabled={busy||uploadCount>0} style={{border:0,padding:0,margin:0}}>{sections.map(s=><section className="review-section" key={s.id}><h2>{s.title}</h2>{s.fields.filter(f=>visible(f,answers)).map(f=><Question key={f.id} field={f} value={answers[f.id]||""} userId={session.user.id} onChange={v=>update(f.id,v)} onError={setError} onUploading={v=>setUploadCount(n=>n+(v?1:-1))}/>)}</section>)}</fieldset>
